...
# database and save as Parquet files.
#
# USAGE: This script is designed to run on the HPC cluster at
# Imperial College London
#
# RUN REQUIREMENTS:
# - Requires access to the 'hitachi' database, which is hosted
...
#
# OUTPUTS:
# - Parquet files containing meter reading data for each year, saved
#   in the specified directory.
# - The files should be named in the format
#   'meter_readings_YEAR_TIMESTAMP.parquet'.
# - In this case, we expect data for the years 2021, 2022,
#   and 2023 to be processed.
# ─────────────────────────────────────────────────────────────────────────────
...
os.makedirs(meter_readings_directory, exist_ok=True)
...
engine = create_engine(
    ...
    pool_size=pool_size,
    # max overflow defines what number if any extra connections are allowed
    # beyong the pool_size
    max_overflow=0,
    # pool pre ping executes a lightweight query when enabled that tests the
    # connection before using it, ensuring that stale or closed connections
    pool_pre_ping=True,
    # pool recycle closes and reopens connections so they don't become stale
    pool_recycle=3600,
    connect_args={
