# ────────────────────────────────────────────────────────────────────────────
# FILE: step1_meter_readings_processing_split_to_quarterly.py
#
...
#   'meter_readings_YEAR_Q#_TIMESTAMP.parquet'. or
#   'meter_readings_CITY_YEAR_Q#_TIMESTAMP.parquet'
# - In this case, we expect data for
...
def split_and_save_by_quarter(
    ...
    try:
        ...
        # extract different parts of filename
        ...
        for q in quarters:
            ...
            # filter and save the data for the current quarter
            (
...
# Assign files to this specific process using round-robin distribution
if file_list:
...
for filename in tasks:
    ...
    # Split and save the data by quarter
    split_and_save_by_quarter(file_path=input_path,
                              output_dir=meter_readings_directory)
    # note there is a try except loop inside the split_and_save_by_quarter
